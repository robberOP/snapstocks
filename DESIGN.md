...
**Selecting a result always goes to that stock's fundamentals page.** There
is no per-ticker fundamentals page yet — every entry's `fundamentals_url`
points at the one shared `stock-analysis/fundamentals/template.html` — so
right now every search result lands on the same (Zaggle) sample data
regardless of which stock you picked. That's a known, temporary state, not
a bug: it's what makes the search usable today for the full ~750-symbol
universe while fundamentals are still being built out per ticker (see
`template.html`'s own header comment). A result that also has a deep-dive
report shows a `.pill-ai` "Deep-dive" badge; clicking that badge
...
| Stock Analysis | Fundamentals (template) | ✅ Built, sample data (Zaggle) — one shared template, not yet per-ticker |
