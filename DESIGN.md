...
**A new stock's fundamentals or deep-dive page**: copy
`stock-analysis/fundamentals/template.html` or `deep-dive/zaggle.html`
respectively, rename to the ticker (e.g. `hdfcbank.html`), and replace the
data blocks — the class names/markup are the reusable part, the numbers are
not. In production these two templates should be server-rendered from the
yfinance pull rather than hand-copied; the HTML structure is what stays fixed
per the data contract implied by the sample markup.
...
| Sector Analysis | Momentum Chart | ✅ Built, sample data |
| Sector Analysis | Momentum Table | ✅ Built, sample data |
...
"Sample data" means the numbers are illustrative for the design, generated
client-side (`assets/js/sector-momentum-data.js`) or hand-typed — none of it
is a live feed yet. `data/sector_momentum.sample.json` documents the shape
the real pipeline should emit to replace the sector-momentum sample generator.
