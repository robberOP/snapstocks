...
`.search-field` · `table.data` (+ `.mag-cell` for diverging-tinted numeric
...
└── stock-analysis/
    ...
    └── deep-dive/
        ├── zaggle.html               # SnapStocks-chrome SHELL — THE pattern for any deep-dive page
        ├── optiemus.html             # shell, same pattern
        ├── nh.html                   # shell, same pattern
        └── reports/
...
### Deep-dive reports are framed, not inlined

Deep-dive reports (`deep-dive/reports/*.html`) are generated by a separate,
independent process — each one is a **complete, self-contained HTML
...
**A new stock's fundamentals page**: copy
`stock-analysis/fundamentals/template.html`, rename to the ticker (e.g.
`hdfcbank.html`), and replace the data blocks — the class names/markup are
the reusable part, the numbers are not. In production this template should
be server-rendered from the yfinance pull rather than hand-copied; the HTML
structure is what stays fixed per the data contract implied by the sample
markup.
...
2. Copy `stock-analysis/deep-dive/zaggle.html` to
   `stock-analysis/deep-dive/<ticker>.html`. Update the `<title>`, the
   breadcrumb's current-page text, the two `aria-current`/subnav hrefs, and
   the iframe's `src`/`title` to point at the new report file. Everything
   else — chrome, `.report-frame-wrap`, the three `<script>` tags — is
   copy-unchanged.
3. Link to `stock-analysis/deep-dive/<ticker>.html` (the shell, never the
   raw file under `reports/`) from wherever the stock is discoverable —
   the fundamentals page's "Deep-dive report available" pill, the
   `stock-analysis/index.html` listing, etc.
...
| Stock Analysis | Fundamentals (template) | ✅ Built, sample data (Zaggle) |
| Stock Analysis | Deep-Dive (Zaggle example) | ✅ Built, sample content |

"Sample data" means the numbers are illustrative for the design, hand-typed —
still true for the two Stock Analysis templates. The two Sector Analysis
